...
/*******************/
{
	...
	VERIFY( sscanf_s( "aaaa", "%s", &buf, 4 ) == 0 );
	...
	// behavior is undefined if the number of conversion specifications
    // in format  exceeds  the  number  of  pointer  arguments
...
/********************************/
{
    ...
    printf( "All tests successfully completed (%s).\n", strlwr( argv[0] ) );
#ifdef __SW_BW
